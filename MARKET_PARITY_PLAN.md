...
- [x] **C5. Virtual keys (gateway parity)**: per-agent/per-project gateway keys with own budget + model allowlist (LiteLLM/Portkey's most-used enterprise feature). Tables: extend `governance_budgets` + new `gateway_keys`. ✅ *(audit 2026-07-03: shipped as GW1 below — `server/gateway/keys.ts` with modelAllowlist + per-key daily cap, `gateway_keys` table live with issued keys; this box was stale)*
