...
import {
  ...
  gatewaySetRouteOverrideHandler,
...
if (method === "GET" && pathname === "/api/stream") {
  ...
  const incidentEscalateMatch = pathname.match(/^\/api\/incidents\/([^/]+)\/escalate$/);
  if (method === "POST" && incidentEscalateMatch) {
    const denied = requireMutation(req);
    if (denied) return denied;
    return incidentEscalateHandler(decodeURIComponent(incidentEscalateMatch[1]), req);
  }
  ...
  if (method === "POST" && pathname === "/api/gateway/route-override") {
    const denied = requireMutation(req);
    if (denied) return denied;
    return gatewaySetRouteOverrideHandler(req);
  }
  if (method === "DELETE" && pathname === "/api/gateway/route-override") {
    const denied = requireMutation(req);
    if (denied) return denied;
...
const geminiStopMatch = pathname.match(/^\/api\/gemini\/sessions\/([^/]+)\/stop$/);
  ...
  if (method === "POST" && pathname === "/api/reports/system-labor") {
    if (!checkToken(req)) return unauthorized();
    const denied = requireMutation(req);
    if (denied) return denied;
    try {
      return Response.json(await generateWeeklySystemLaborReport({ force: true }));
    } catch (err) {
      const errMsg = err instanceof Error ? err.message : String(err);
      return new Response(JSON.stringify({ error: errMsg }), {
        status: 500,
  ...
  if (method === "POST" && pathname === "/api/reports/sla-uptime") {
    if (!checkToken(req)) return unauthorized();
    const denied = requireMutation(req);
    if (denied) return denied;
    try {
