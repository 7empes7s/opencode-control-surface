...
export function getChainHead(db?: Database): { rowHash: string; ts: number; rowId: number } | null {
  const d = db ?? getDashboardDb();
  if (!d) return null;
  try {
    const row = d.query(
      `SELECT id, ts, row_hash FROM action_audit WHERE row_hash IS NOT NULL ORDER BY id DESC LIMIT 1`
    ).get() as { id: number; ts: number; row_hash: string } | null;
    if (!row) return null;
...
export function appendAudit(
  ...
  try {
    db.query(
      `UPDATE action_audit SET prev_hash = ?, row_hash = ? WHERE id = (SELECT MAX(id) FROM action_audit WHERE row_hash IS NULL)`
    ).run(prevHash, rowHash);
  } catch {
    // non-fatal: chain integrity writes should never block the audit write
  }
}

export function verifyChain(db?: Database, limit = 1000): { ok: boolean; firstBadId?: number; checkedCount: number } {
  const d = db ?? requireDb();
  const rows = d.query(
    `SELECT id, prev_hash, row_hash FROM action_audit WHERE row_hash IS NOT NULL ORDER BY id ASC LIMIT ?`
  ).all(limit) as Array<{ id: number; prev_hash: string | null; row_hash: string }>;
...
export function anchorChain(db?: Database, date?: string): void {
  const d = db ?? getDashboardDb();
  if (!d) return;
  const head = getChainHead(d);
  if (!head) return;
  const anchorDate = date ?? new Date().toISOString().slice(0, 10);
  const rowCount = (d.query(`SELECT COUNT(*) as c FROM action_audit WHERE row_hash IS NOT NULL`).get() as { c: number } | null)?.c ?? 0;
