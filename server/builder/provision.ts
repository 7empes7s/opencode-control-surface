...
function findPlanFiles(projectRoot: string): string[] {
  ...
  try {
    for (const entry of readdirSync(projectRoot, { withFileTypes: true })) {
      ...
      if (name.includes("plan") || name.includes("roadmap") || name === "readme.md") {
...
function createAgentsMd(projectRoot: string, name: string, description: string): { ok: boolean; path: string } {
  ...
  if (existsSync(agentsPath)) return { ok: true, path: agentsPath };
  const template = `# ${name}
...
function createProjectSkill(projectRoot: string, name: string): { ok: boolean; path: string } {
  ...
  try {
    ...
    if (existsSync(skillPath)) return { ok: true, path: skillPath };
    const content = `---
...
function createValidationProfileFile(projectRoot: string, input: ProvisionInput, commands: string[]): { ok: boolean; path: string } {
  const profileDir = join(projectRoot, ".opencode");
  ...
  try {
    ...
    const profile = {
      ...
      notes: [
        "Generated by Dashboard V4 Builder provisioning.",
        "Edit this file or the Builder workflow validation profile when project checks change.",
      ],
      metadata: {
        name: input.name,
    ...
    return { ok: false, path: profilePath };
  }
}

function slugifyName(name: string): string {
  return name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "builder-project";
}
...
function createRuntimeScaffold(projectRoot: string, input: ProvisionInput): { files: string[]; warnings: string[] } {
  const slug = slugifyName(input.name);
  const files: string[] = [];
  const warnings: string[] = [];
  const dockerfilePath = join(projectRoot, "Dockerfile");
  const composePath = join(projectRoot, "compose.yaml");
  const servicePath = join(projectRoot, ".opencode", "systemd", `${slug}.service`);
...
`;
  ...
  for (const [path, content] of [
    ...
    if (writeFileIfMissing(path, content)) files.push(path);
    else warnings.push(`runtime scaffold file creation failed: ${path}`);
  }
...
export function provisionProject(input: ProvisionInput): ProvisionResult {
  ...
  if (input.repoUrl) {
    ...
    if (!cloneResult.ok) {
      return { ok: false, projectRoot, name: input.name, provisioned: { cloned: false, gitInitialized: false, agentsMd: false, planFile: null, vaultNote: false, skillFile: false, validationProfileFile: null, runtimeScaffoldFiles: [] }, validationCommands: [], workflows: [], warnings, error: cloneResult.error };
    }
    cloned = true;
    ...
    try { mkdirSync(projectRoot, { recursive: true }); } catch (e) {
      return { ok: false, projectRoot, name: input.name, provisioned: { cloned: false, gitInitialized: false, agentsMd: false, planFile: null, vaultNote: false, skillFile: false, validationProfileFile: null, runtimeScaffoldFiles: [] }, validationCommands: [], workflows: [], warnings, error: `could not create project root: ${e instanceof Error ? e.message : String(e)}` };
    }
  }
  ...
  const validationProfileResult = createValidationProfileFile(projectRoot, input, validationCmds);
  if (!validationProfileResult.ok) warnings.push("validation profile file creation failed");

  const runtimeScaffold = input.runtimeScaffold
    ? createRuntimeScaffold(projectRoot, input)
    : { files: [], warnings: [] };
  warnings.push(...runtimeScaffold.warnings);
  ...
  return {
    ...
    provisioned: {
      ...
      validationProfileFile: validationProfileResult.ok ? validationProfileResult.path : null,
      runtimeScaffoldFiles: runtimeScaffold.files,
    },
